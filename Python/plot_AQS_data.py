...
###############################################################################
outPut = 'HMS_gridded+' + gfed + '_emissions_' + species
outPutSavename = dataDir + outPut
#with open(outPutSavename + '.pickle') as f:  
#    HMS_emissions, GFED_emissions, lon, lat, \
#    gridYear, gridMonth = pickle.load(f)
    
ncFile = Dataset(outPutSavename + '.nc', 'r')
HMS_emissions = np.array(ncFile.variables['HMS'])
GFED_emissions = np.array(ncFile.variables['GFED'])
lon = np.array(ncFile.variables['longitude'])
lat = np.array(ncFile.variables['latitude'])
gridYear = np.array(ncFile.variables['year'])
gridMonth = np.array(ncFile.variables['month'])
ncFile.close()

# Make nice montlhy date object for x-axis of plotting monthly grid
# time series  

t = []
for i in range(len(gridYear)):
    # Represent each month by the 15th for middle-plotting purposes 
    t.append( date(gridYear[i], gridMonth[i], 15) ) # 15th is month middle
t = np.array(t)
...
def plotMonthlyHMS_emissions():

    nMonth = HMS_emissions.shape[2]
    for i in range(nMonth):
    
        # Subset the data
        data = HMS_emissions[:,:,i]    
        
        # Mask data such that zero values are NOT plotted 
        data = ma.masked_where(data == 0, data)
    
        # Create nice labels and savenames for figures 
        figureName = figureSaveDir + 'HMS_emissions_' + str(gridYear[i]) +  \
                      '_' + str(gridMonth[i]) + '.png'
        
        figureTitle = 'Gridded HYSPLIT Points ' + str(gridYear[i]) + \
                      '-' + str(gridMonth[i]) 
        ...
        cs = m.pcolor(x, y, data, cmap='Reds', vmin=1, vmax=200)
        # add colorbar.
        cbar = m.colorbar(cs, location='bottom',pad="5%")
        cbar.set_label('Observed hms emissions (hrs)', fontsize=18)
        # add title
        plt.title(figureTitle, fontsize=20)
        plt.savefig(figureName, format='png')
        plt.close(fig)
        
# Execute the function if desired 
if makeMonthlyFigures:
    plotMonthlyHMS_emissions()
